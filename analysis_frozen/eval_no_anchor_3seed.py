#!/usr/bin/env python
"""no-anchor 3-seed: 代理恢复MAE跨seed一致性."""
...
def main():
    ...
    print("="*60, flush=True)
    print("no-anchor 3-seed proxy recovery", flush=True)
    print("="*60, flush=True)
    ...
    per_seed = {}
    for seed in SEEDS:
        ckpt = REPO / f"runs/v4/abl_no_anchor_s5_s{seed}/best.pt"
        if not ckpt.exists():
        ...
        # 代理恢复: concept_mean vs weak_concept_target
        all_diff = []
        with torch.inference_mode():
            for batch in loader:
                ...
                # weak_concept_target / mask 由 model.forward() 生成 (plf_ogt_v4.py 821-822),
                # 不在 batch 里 — 必须从 out 提取.
    ...
    json.dump(summary, open(OUTDIR / "no_anchor_3seed.json", "w"), indent=2, ensure_ascii=False, default=float)
    print(f"保存: {OUTDIR / 'no_anchor_3seed.json'}", flush=True)
