#!/usr/bin/env python
"""Full S3 recompute: sofa total MAE + TRUE macro MAE with cluster-bootstrap CIs.
...
Saves frozen predictions npz + JSON with sofa/macro/discrimination CIs at 1/3/6/12h.
"""
import os
import sys
import json
...
OUTPATH = Path(r"F:/MIMIC3_1/V13/results/v4/recompute_s3_full_ci.json")
OUTPRED = Path(r"F:/MIMIC3_1/V13/results/v4/recompute_frozen_predictions.npz")
SEEDS = [42, 52, 62]
...
def main():
    ...
    np.savez_compressed(
        OUTPRED,
        plf_tcr_organ=plf_tcr_organ.astype(np.float32),
        plf_co_organ=plf_co_organ.astype(np.float32),
        plf_tcr_logits=plf_tcr_logits.astype(np.float32),
        plf_co_logits=plf_co_logits.astype(np.float32),
        organ=organ.astype(np.float32),
        omask=omask.astype(np.float32),
        delta=delta.astype(np.float32),
        cmask=cmask.astype(np.float32),
        stays=stays.astype(np.int32),
    )
    print("predictions saved", OUTPRED, flush=True)
    ...
    for h, hi in HORIZONS:
        ...
        sofa_t_v = sofa_mae_per_anchor(pred_t, true_h, mask_h)
        sofa_c_v = sofa_mae_per_anchor(pred_c, true_h, mask_h)
        sofa_t = float(sofa_t_v.mean())
        sofa_c = float(sofa_c_v.mean())
        ...
        for _ in range(N_BOOT):
            ...
            b_c.append(mc)
            b_d.append(mt - mc)
            st = float(sofa_t_v[ix].mean())
            sc = float(sofa_c_v[ix].mean())
            bs_t.append(st)
            bs_c.append(sc)
            bs_d.append(st - sc)
        sofa_rows.append({
            "horizon": h, "valid_n": n_v, "n_clusters": n_st,
            "sofa_tcr": sofa_t, "sofa_tcr_ci": percentile_ci(bs_t),
            "sofa_co": sofa_c, "sofa_co_ci": percentile_ci(bs_c),
            "delta_sofa": sofa_t - sofa_c, "delta_sofa_ci": percentile_ci(bs_d),
        })
        macro_rows.append({
            "horizon": h, "valid_n": n_v, "n_clusters": n_st,
            "organ_tcr": [float(x) for x in om_t],
            "organ_co": [float(x) for x in om_c],
            "macro_tcr": macro_t, "macro_tcr_ci": percentile_ci(b_t),
            "macro_co": macro_c, "macro_co_ci": percentile_ci(b_c),
            "delta_macro": macro_t - macro_c, "delta_macro_ci": percentile_ci(b_d),
        })
        print(f"{h}h: n={n_v} cl={n_st} sofa TCR={sofa_t:.4f} co={sofa_c:.4f} "
              f"d={sofa_t - sofa_c:+.4f} (CI {percentile_ci(bs_d)[0]:+.4f}~{percentile_ci(bs_d)[1]:+.4f})", flush=True)
        print(f"    TRUE macro TCR={macro_t:.4f} co={macro_c:.4f} d={macro_t - macro_c:+.4f} "
              f"(CI {percentile_ci(b_d)[0]:+.4f}~{percentile_ci(b_d)[1]:+.4f})", flush=True)
    ...
    for h, hi in HORIZONS:
        ...
        for _ in range(N_BOOT):
            ...
            try:
                ...
                continue
        disc_rows.append({
            "horizon": h, "n_valid": int(mc.sum()), "n_clusters": n_st, "prev": float(yv.mean()),
            "tcr_auroc": a_t, "tcr_auroc_ci": percentile_ci(ba_t),
            "tcr_auprc": ap_t, "tcr_auprc_ci": percentile_ci(bap_t),
            "co_auroc": a_c, "co_auroc_ci": percentile_ci(ba_c),
            "co_auprc": ap_c, "co_auprc_ci": percentile_ci(bap_c),
            "delta_auroc": a_t - a_c, "delta_auroc_ci": percentile_ci(bd_a),
            "delta_auprc": ap_t - ap_c, "delta_auprc_ci": percentile_ci(bd_ap),
        })
        print(f"{h}h disc: n={int(mc.sum())} cl={n_st} prev={yv.mean() * 100:.2f}%", flush=True)
        print(f"    TCR {a_t:.4f} ({percentile_ci(ba_t)[0]:.4f}~{percentile_ci(ba_t)[1]:.4f}) / "
              f"{ap_t:.4f} ({percentile_ci(bap_t)[0]:.4f}~{percentile_ci(bap_t)[1]:.4f}) | "
              f"co {a_c:.4f} ({percentile_ci(ba_c)[0]:.4f}~{percentile_ci(ba_c)[1]:.4f}) / "
              f"{ap_c:.4f} ({percentile_ci(bap_c)[0]:.4f}~{percentile_ci(bap_c)[1]:.4f})", flush=True)

    out = {
        "note": "TRUE macro = mean of 6 per-organ pooled MAE; sofa = per-anchor total-SOFA abs error mean; "
                "delta = TCR - care-off; cluster bootstrap n_boot=2000 seed=42 percentile 2.5/97.5",
        "sofa": sofa_rows,
        "macro": macro_rows,
